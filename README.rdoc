...
==== An even more complicated example (if you need additional attributes passed to the cas login page)

  CASClient::Frameworks::Rails::Filter.configure(
    :cas_base_url         => "https://cas.example.foo/",
    :login_url            => "https://cas.example.foo/login",
    :extra_login_params   => lambda { |controller| { :dynamic => :here } }
  )

calling `login_url` would then generate:

  CASClient::Frameworks::Rails::Filter.login_url
    => "https://cas.example.foo/login?service=bababa&dynamic=here"
